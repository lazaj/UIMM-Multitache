...
int main(void)
{
	...
	printf("Le thread1 (etoile) doit faire au moins 1/3 de son travail (%d de ses %d boucles)\n",
		   ...
		   COUNT);
	printf(
		"Le thread2 (diese) attendra que le thread1 (etoile) ait fait au moins 1/3 de son travail avant dedémarrer le sien\n");
...
void* etoile(void* inutilise)
{
	...
	{
		write(1, &c1, 1);
		// écritune’*’surstdout(descripteur1)
		count++;
...
void* diese(void* inutilise)
{
	...
	while (1)
	{
		write(1, &c1, 1);  // écrit un ’#’ sur stdout (descripteur1)
		count++;
		if (count >= COUNT)
		{
			return (NULL);
